...
func TestWorkerPublishesOneCompleteCardPerRecipient(t *testing.T) {
	...
	if !strings.Contains(payload, "สต็อกคงเหลือ") || !strings.Contains(payload, "ขาย · รายวัน") || !strings.Contains(payload, "deliveryRef") || !strings.Contains(payload, salesRunID.String()) || !strings.Contains(payload, "/app/tenant/"+tenantID.String()) || !strings.Contains(payload, "อัปเดต 10 ก.ค. 2569 15:00") || strings.Contains(payload, "UTC") {
